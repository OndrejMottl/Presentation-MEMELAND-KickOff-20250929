...
# Theme Generation Script for LoQE Presentation
...
generate_colors_scss <- function(
    input_file = here::here("colors.json"),
    output_file = here::here("_colors.scss")) {
  ...
  ui_section <- if (length(ui_colors) > 0) {
    c(
      ...
      "",
      "// HTML-specific UI variables for compatibility",
      ...
      "$link-hover-color: darken($linkHoverColor, 15%);",
      ""
    )
  } else {
    c("")
...
generate_fonts_scss <- function(
    input_file = here::here("fonts.json"),
    output_file = here::here("_fonts.scss"),
...
generate_custom_theme_scss <- function(
    colors_file = here::here("colors.json"),
    fonts_file = here::here("fonts.json"),
    custom_theme_file = here::here("custom_theme.json"),
    output_file = here::here("custom_theme.scss")) {
  ...
  custom_theme_scss_content <-
    c(
      ...
      "// LoQE Custom Theme for Reveal.js Presentations",
      "",
      "/*-- scss:defaults --*/",
      "",
...
generate_color_variables <- function(colors_data) {
  ...
  if ("primary" %in% names(colors_data)) {
    ...
    return(c(
      ...
      paste0("$teal: ", colors_data$teal, ";"),
      paste0("$pistachio: ", colors_data$pistachio, ";"),
      paste0("$seal_brown: ", colors_data$seal_brown, ";"),
      paste0("$dark_green: ", colors_data$dark_green, ";"),
      paste0("$fulvous: ", colors_data$fulvous, ";")
    ))
  }
}
...
generate_html_heading_styles <- function() {
  c(
    "// Heading styles with LoQE brand colors",
    "h1, .h1 {",
    "  color: $headingColor;",
...
generate_html_utility_classes <- function() {
  c(
    "// Utility classes for LoQE brand colors",
...
generate_r_theme <- function(
  ...
  r_theme_content <-
    c(
      ...
      paste0('  teal = "', colors$teal, '",'),
      paste0('  pistachio = "', colors$pistachio, '",'),
      paste0('  seal_brown = "', colors$seal_brown, '",'),
      paste0('  dark_green = "', colors$dark_green, '",'),
      paste0('  fulvous = "', colors$fulvous, '"'),
      ")",
      "",
      ...
      paste0('loqe_primary_sequence <- c("', colors$teal, '", "', colors$pistachio, '", "', colors$seal_brown, '")'),
      "",
      "# Diverging color palette (for diverging data)",
      paste0('loqe_diverging <- c("', colors$dark_green, '", "', colors$white, '", "', colors$fulvous, '")'),
      "",
      "# Text and background colors",
      paste0('loqe_text_color <- "', colors$black, '"'),
      paste0('loqe_background_color <- "', colors$white, '"'),
      paste0('loqe_accent_color <- "', colors$fulvous, '"'),
      "",
      "# Define typography (based on LoQE brand guidelines)",
      "# Using registered font names from sysfonts",
      paste0('loqe_base_font <- "', fonts$system$internalBodyName, '"'),
      paste0('loqe_heading_font <- "', fonts$system$internalHeadingName, '"'),
      paste0('loqe_mono_font <- "', fonts$system$internalMonoName, '"'),
      "",
      ...
      "  ggplot2::theme(",
      "    # Text elements",
      paste0('    text = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '"),'),
      paste0('    plot.title = ggplot2::element_text(family = loqe_heading_font, colour = "', colors$dark_green, '", face = "', fonts$ggplot$plotTitleFace, '", size = ggplot2::rel(', fonts$ggplot$plotTitleSize, ')),'),
      paste0('    plot.subtitle = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '", size = ggplot2::rel(', fonts$ggplot$plotSubtitleSize, ')),'),
      paste0('    axis.title = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '", size = ggplot2::rel(', fonts$ggplot$axisTitleSize, ')),'),
      paste0('    axis.text = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '", size = ggplot2::rel(', fonts$ggplot$axisTextSize, ')),'),
      paste0('    legend.title = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '", size = ggplot2::rel(', fonts$ggplot$legendTitleSize, ')),'),
      paste0('    legend.text = ggplot2::element_text(family = loqe_base_font, colour = "', colors$black, '", size = ggplot2::rel(', fonts$ggplot$legendTextSize, ')),'),
      paste0('    strip.text = ggplot2::element_text(family = loqe_heading_font, colour = "', colors$dark_green, '", face = "', fonts$ggplot$stripTextFace, '"),'),
      "    ",
      "    # Background elements",
...
generate_fonts_html <- function(
    input_file = here::here("fonts.json"),
    output_file = here::here("fonts-include.html")) {
...
generate_custom_theme_font_overrides <- function(fonts) {
  c(
    ...
    "$headingTextTransform: none; /* LoQE uses title case, not uppercase */",
...
generate_custom_theme_blockquote_styles <- function(custom_theme) {
  c(
    ...
    paste0("  border-left: ", custom_theme$blockquote$blockquoteBorderWidth, " solid $teal;"),
...
generate_custom_theme_exercise_overrides <- function() {
  c(
    ...
    ".reveal .slides section.exercise * a {",
    "  color: $backgroundColor !important; /* Pure white for maximum contrast against Teal */",
    "  text-decoration: underline !important;",
